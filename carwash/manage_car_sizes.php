...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add') {
    ...
    if ($name === '') {
        ...
        $stmt = $conn->prepare('INSERT INTO car_sizes (name) VALUES (?)');
        $stmt->bind_param('s', $name);
        if ($stmt->execute()) { $messages[] = "Size \"$name\" added successfully."; } else { $errors[] = 'Insert failed: ' . $conn->error; }
        $stmt->close();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete') {
    $id = (int)($_POST['id'] ?? 0);
    if ($id > 0) {
        $stmt = $conn->prepare('DELETE FROM car_sizes WHERE id = ?');
        $stmt->bind_param('i', $id);
        if ($stmt->execute()) { $messages[] = 'Size deleted successfully.'; } else { $errors[] = 'Delete failed: ' . $conn->error; }
        $stmt->close();
    }
}

$result = $conn->query('SELECT id, name, created_at FROM car_sizes ORDER BY name');
...
<style>
    ...
    .cs-table thead tr {
        background: linear-gradient(135deg, #1B3FA0, #00AEEF);
    }
    .cs-table th {
...
<div class="cs-page">
    ...
    <!-- Page Title -->
    <div class="cs-title">
        <div class="cs-title-icon"><i class="fas fa-car-side"></i></div>
        <div>
            <h1>Manage Car Sizes</h1>
            <p>Add and manage vehicle size categories used during car wash registration.</p>
        </div>
    </div>
    ...
    <div class="cs-card">
        <div class="cs-card-header">
            <h2><i class="fas fa-plus-circle"></i> Add New Size</h2>
        </div>
        <div class="cs-card-body">
            <form method="post" class="cs-add-form">
                ...
                <div class="cs-field">
                    ...
                    <input type="text" id="size_name" name="name" placeholder="e.g. Small, Medium, Large, Extra Large" required autocomplete="off">
                </div>
    ...
    <div class="cs-card">
        <div class="cs-card-header">
            <h2><i class="fas fa-list-ul"></i> Existing Sizes</h2>
            <span class="cs-badge"><?php echo count($rows); ?> Size<?php echo count($rows) !== 1 ? 's' : ''; ?></span>
        </div>

        <?php if (empty($rows)): ?>
            <div class="cs-empty">
                <i class="fas fa-car"></i>
                <strong>No sizes added yet</strong>
                Use the form above to add your first car size.
            </div>
        <?php else: ?>
        <div class="cs-table-wrap">
            <table class="cs-table">
                <thead>
                    <tr>
                        <th>#</th>
                        <th><i class="fas fa-tag" style="margin-right:6px;opacity:.8;"></i>Size Name</th>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="size-badge">
                                <i class="fas fa-car" style="font-size:.75rem;"></i>
                        ...
                        <td style="text-align:center;">
                            <form method="post" style="margin:0;display:inline;"
                                  data-confirm="Delete size &quot;<?php echo htmlspecialchars($r['name'], ENT_QUOTES); ?>&quot;? This cannot be undone.">
